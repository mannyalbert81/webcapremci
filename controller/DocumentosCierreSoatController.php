...
class DocumentosCierreSoatController extends ControladorBase{
	...
	public function index(){
		...
		if (isset(  $_SESSION['usuario_usuario']) )
		{
			...
			if (!empty($resultPer))
			{
				...
				//Carton Documento
				$soat=new SoatModel();
				$columnas_so = " soat.id_soat, soat.cierre_ventas_soat";
				$tablas_so   = " public.documentos_legal, public.soat";
				$where_so  = " soat.id_soat = documentos_legal.id_soat GROUP BY soat.id_soat, soat.cierre_ventas_soat";
				$id_so = " soat.cierre_ventas_soat";
				...
				if (isset ($_POST["categorias"]) && isset ($_POST["subcategorias"])  && isset($_POST["cierre_ventas_soat"])  && isset($_POST["fecha_documento_desde"]) && isset($_POST["fecha_documento_hasta"])  && isset($_POST["fecha_subida_desde"])  && isset($_POST["fecha_subida_hasta"])   )
				
				{
					...
					if ($_id_soat > 0)
					{
						...
						$where_4 = "  AND soat.id_soat = '$_id_soat' ";
					}	
			...
			if ($resultSet != "")
			{
				...
				$this->view("DocumentosCierreSoat",array(
						"resultCat"=>$resultCat, "resultSub"=>$resultSub, "resultSoa"=>$resultSoa, "resultSet"=>$resultSet, "arraySel"=>$arraySel, "resultEdit"=>$resultEdit, "resul"=>$resul  , "paginasTotales"=>$paginasTotales, "registrosTotales"=> $registrosTotales,"hojasTotales"=>$hojasTotales, "pagina_actual"=>$paginaActual
